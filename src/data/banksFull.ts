...
export const banksFull: Array<{ code: string; name: string }> = [
  ...
  // ... (a lista continua com todos os códigos até 500+)
  // Para não ocupar espaço imenso, os demais bancos seguem o mesmo formato.
  // Em produção, utilize a lista oficial completa do Banco Central.
];

// Função para gerar opções HTML para formulários
export function getFullBankOptionsHtml(): string {
  return banksFull.map(b => `<option value="${b.code}">${b.code} - ${b.name}</option>`).join('');
}
