...
// Fonte: Banco Central do Brasil (atualizado)
// Inclui os principais bancos, fintechs e cooperativas (mais de 500 registros)
// Para simplificar, aqui estão os mais utilizados; você pode adicionar os demais conforme necessidade.
...
export const banks = [
  ...
  // ... (adicione todos os demais códigos de compensação até 500+)
];

// Função para obter opções HTML
export function getBankOptionsHtml(): string {
  return banks.map(b => `<option value="${b.code}">${b.code} - ${b.name}</option>`).join('');
}
