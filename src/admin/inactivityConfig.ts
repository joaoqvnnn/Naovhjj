...
export async function setInactivityDays(ctx: Context) {
  ...
  await startCapture(ctx, 'inactivity_days', 'Digite o número de dias:', {
    ...
    onSuccess: async (ctx, value) => {
      ...
      await prisma.setting.upsert({ where: { key: 'inactivity_days' }, update: { value: days }, create: { key: 'inactivity_days', value: days } });
      await ctx.editMessage('✅ Inatividade atualizada.');
