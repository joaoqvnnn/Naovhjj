...
export async function toggleTranscription(ctx: Context) {
  ...
  await prisma.setting.upsert({ where: { key: 'transcription_enabled' }, update: { value: !current }, create: { key: 'transcription_enabled', value: !current } });
  await ctx.editMessage('✅ Transcrição atualizada.');
  await showTranscriptionConfig(ctx);
}

export async function setTranscriptionKey(ctx: Context) {
  if (!(await isAdmin(ctx))) return;
  await startCapture(ctx, 'transcription_key', 'Digite a OpenAI API Key:', {
