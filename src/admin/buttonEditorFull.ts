...
export async function viewButtonConfig(ctx: Context, key: string) {
  ...
  const inline: any[] = [];
  // Cada botão existente ganha opção de editar/remover
  buttons.forEach((btn, i) => {
    inline.push([{ text: `✏️ ${btn.text}`, callback_data: `btnedit_${key}_${i}` }]);
  ...
  inline.push([{ text: '⏮️ Voltar', callback_data: 'button_list' }]);
...
export async function addButton(ctx: Context, key: string) {
  if (!(await isAdmin(ctx))) return;
  await startCapture(ctx, `btnadd_${key}`, 'Digite no formato: Texto | callback_data (ex: Comprar | comprar_1):', {
...
export async function editButton(ctx: Context, key: string, index: number) {
  ...
  await startCapture(ctx, `btnedit_${key}_${index}`, `Editando "${buttons[index].text}". Digite novo formato: Texto | callback_data`, {
    validate: async (input) => {
      const parts = input.split('|').map(s => s.trim());
